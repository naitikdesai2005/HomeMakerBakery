...
const Users = () => {
  ...
  // Filter out users based on search query
  ...
  // Calculate total orders excluding common users (assuming `user.role` or similar determines if the user is common)
  const totalOrders = filteredUsers.reduce((total, user) => {
    if (user.role !== "common") {
              ...
              {currentItems.map((user, index) => (
                ...
                >
                  ...
                  <td className="py-4 px-4">
                    {user.role !== "common" ? user.orders?.length : 0}
                  </td>
                </tr>
              ))}
            </tbody>
